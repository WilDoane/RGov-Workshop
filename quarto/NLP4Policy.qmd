...
format: 
  revealjs:
    ...
    footer: R Government & Public Policy Conference
    smaller: false
...
<https://github.com/WilDoane/RGov-Workshop>
...
grViz("
  digraph G {
    node [shape=record, fontcolor=black, fillcolor=white, style=filled];
    edge [color=white];
    rankdir='LR'
    bgcolor=transparent
...
## We're Not Doing Everything Today

Day      | Time  | Speaker                 | Title
-------- | ----- | ----------------------- | --------------------------------------------------
Thursday | 10:40 | Abigail Haddad | What Job Is This, Anyway?: Using LLMs to Classify USAJobs Data Scientist Listings
&nbsp; | 11:05 | Jared P.  | I Wrote this Talk with an LLM
&nbsp; | 11:30 | Soubhik Barari | LocalView: Scaling up the Analytics of Local Politics with R
Friday | 11:40 | Selen Stromgren & Danielle Larese | Deterministic Extraction vs. Probabilistic Extrapolation: A Pilot for R-Enabled Augmentation of Information Retrieval by Humans
&nbsp; | 12:05 | Vivian Peng & David Cyprian | Using Large Language Models in Production: Hype vs Reality 
...
## We're Not Stealing Intellectual Property

![Microsoft Github's Copilot](images/github-copilot.jpg)

## We're Not Being Unkind to Others
...
* Documents of interest may use specialized language
...
## Acquire Raw Data
* Use existing text files (readLines)
* Extract text from PDF (pdftools, tabulizer)
* Extract text from MS Word (antiword, officer, )
* Extract text from MS Excel (readr, readxl)
* Web scraping (httr, rvest)
* Via APIs (httr)
* From databases (ODBC)
...
* Organize code and data according to practices in <https://r-pkgs.org/>
...
Converting textual data into keyword extraction, frequency counts, co-occurrence matrices, similarity scores, topic models.
...
Insight is limited by the letters and words in the documents.
...
&nbsp; | Do Nothing | Do Less | Do the Same | Do More | Do Something Else
---------------- | ------- | ------- | --------- | ------- | ---------------
...
The UTF-8 Byte Order Marker (BOM) is optional, but if present is `EF BB BF`

UTF-16 uses 2, 3, or 4 bytes

The UTF 16 Byte Order Marker (BOM) is (a usually invisible) pair of bytes (UTF-16LE: `FF FE` or UTF-16BE: `FE FF`) at the start of a file indicating whether the byte order is encoded as `00 0a` or `0a 00`. See: 
...
There are also some <https://invisible-characters.com/> in Unicode I like to be aware of, but they come up less often: 

### hexView

A single text file saved from BBEdit using 4 different character encodings. hexView::viewRaw() allows me to inspect the bytes contained in a file without any imposed interpretation: what's really there?
...
### Setting Encoding
R has two ways to set encoding depending on the claim you're trying to make: either you're saying:

* fileEncoding: "I want you to re-encode the content to be..."
* encoding: "Assume this file is already using the encoding..."

### read.csv 

Notice that the `<ff><fe>` mentioned in the error message is telling you that you're trying to read a UTF-16 encoded file---and that it's little endian, at that!

Notice also that read.csv stopped reading the file after the first `t` character. That's because read.csv was likely defaulting to ASCII and encountered bytes `FF FE 74 00` and the `00` NULL byte was assumed to mean END OF FILE.

```{.r}
hex(here("data-raw", "plaintext-utf-16-LE.csv"))
```

```{r}
try({read.csv(here("data-raw", "plaintext-utf-16-LE.csv"))})
```
...
```{.r}
readRaw(here("data-raw", "plaintext-utf-16-LE.csv"))
```

```{r}
try({readRaw(here("data-raw", "plaintext-utf-16-LE.csv"))})
```


### w/fileEncoding LE

Notice here that the declared fileEncoding _does_ match the saved encoding of the file being read, so we see the expected result:

```{r .bigger}
#| echo: true
read.csv(here("data-raw", "plaintext-utf-16-LE.csv"), fileEncoding = "UTF-16LE")
```

### w/fileEncoding BE

Notice here that the declared fileEncoding does _not_ match the saved encoding of the file being read, which leads it interesting results:

```{r}
#| echo: true
read.csv(here("data-raw", "plaintext-utf-16-LE.csv"), fileEncoding = "UTF-16BE")
```

### w/encoding

```{.r}
read.csv(here("data-raw", "plaintext-utf-16-LE.csv"), encoding = "UTF-16")
```

```{r}
try({read.csv(here("data-raw", "plaintext-utf-16-LE.csv"), encoding = "UTF-16")})
```
...
* Designed by Stephen Kleene in the 1950s
...
```
:::{.smaller}
`*` and `+` are _greedy_... they'll match as many characters as then can: c.*t given "catat" will match the full text "catat" since it starts with a c and ends with a t.
`*?` and `+?` are _lazy_ versions... they'll match as few characters as they can: c.*t given "catat" will match just on the "cat" portion.
:::
...
The backslash \ is used to _escape_ the meaning of a character. For example, if you wanted to match a literal question mark how would you do it? Remember: the question mark is used as a quantifier meaning _zero or one of the previous element_.
...
* U.S. phone number including area code
...
* `([0-9]{3}) [0-9]{3}-[0-9]{4}`
* `[0-9]{3}-[0-9]{2}-[0-9]{4}`
* `[-.+a-zA-Z0-9]+@[-.+a-zA-Z0-9]+\\.(com|edu|gov|org)`
* `[pPbBdD][aA@][sS\$][sS\$][wWmM][oO0][rR][dDpPbB]`
...
* matching number of parentheses, braces, brackets, quotation marks, etc.
...
# Using the Federal Register {.smaller}
...
# Getting a List of Federal Agencies via API {.smaller}
<https://www.federalregister.gov/developers/documentation/api/v1>
...
# Getting a Presidential Executive Order via API {.smaller}
...
# Getting a Set of Presidential Executive Orders via API {.smaller}
...
<https://dl.acm.org/doi/10.5555/972470.972475>
...
The analysis of textual data, usually without the aid of machine learning. Frequency counts form the basis for most text analytics and machine learning methods.
...
TF is the absolute frequency of a term within a document. 

TF can be normalized: That is, for any given term, the frequency of the term divided by the number of terms in the document.
...
The number of times a given term occurs in a document times the log ratio of the number of documents in which the term appears.
...
This approach is sometimes referred to as the _bag of words_ model.
...
Topics should be considered an exploratory tool, not an absolute truth.

Consult subject matter experts (SMEs) to determine usefulness of the topic model and to suggest meaningful labels for each.

Consider using a *card sorting task* using the top *n* documents in each topic: do SMEs sort the docuemnts into the same sets as the topic model?
...
## R for Public Policy
* <https://www.lecy.info/r-for-public-policy/>
* <https://link.springer.com/book/10.1007/978-3-030-71352-2>
* <https://link.springer.com/book/10.1007/978-3-030-71352-2>
